...
At this point let's focus on our `folder` function that we pass `foldBack`. Our `folder` function
get the argument x` and `acc`. At any point in time. What does that values represents exactly?
...
And that is bad. Because we have to start with `1` and we need to pass an `acc` to the `folder` 
...
function that the caller provided us `f` and pass it the *current value* and the
*right-accumulator*. At this point in time we have the current value inside of `x`,
but we don't have the *right-accumulator*. What we now do, we just create a *innerFunction*
...
Let's look at our `f 2 racc` call. What does it anyway do and produce as a output? Or better yet,
...
value that can be used to call our previous function that we created.
...
Here we have the same. The only difference is that this time `cont` is a function the we created
inside a function in a previous loop. Let's shorten the example a little bit. In function composition
we just ended with `let compose f g x = g (f x)`. Here we now have
...
One remaining part is once we finished our looping. To understand better what we have to do here.
Let's actually try to understand what we exactly build. In every step we create a new function
...
function the we create basically builds the *right-accumulator* that is need for the *previous-step*.
Once we looped through our whole data-structure, we ended up with a final function. The only thing
we need to start our function chain is to pass it a *right-accumulator*.

But wait, at this point we have the *right-accumulator*! At the end we have to pass it the
*right-accumulator* of our last element. We only have the list `[1;2;3;4]` so what is the
*right-accumulator* of this one? The *initial-accumulator* that the user passed to `foldBack`!

So here is our answer. Once we are at the end of our list we have a function and we need to pass it
...
Now `initCont` also must be a *starting function*. But what do we expect it to do? Well nothing.
...
it that touches every element again. A *continuation function* here can result in higher
memory consumption and could be slower, *reversing* in the end can really be faster and easier
...
we don't care how a function work. Functions are black-boxes. We only care for the input and
